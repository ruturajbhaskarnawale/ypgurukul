...
export const DocumentVaultStep: React.FC<StepProps> = ({ data, update, onNext, onBack }) => {
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {docs.map((doc) => (
          <div 
            ...
            className={`cursor-pointer group relative p-10 rounded-[2.5rem] border-2 transition-all duration-500 flex flex-col items-center justify-center text-center gap-6 ${data.documents[doc.key as keyof typeof data.documents] ? 'bg-foreground border-foreground scale-105 shadow-2xl' : 'bg-muted/10 border-dashed border-border hover:border-muted-foreground hover:bg-muted/20'}`}
          >
      ...
      <div className="p-8 bg-primary/5 border border-primary/20 rounded-3xl flex items-center gap-6">
      ...
      <div className="flex justify-between items-center pt-12 border-t border-border mt-12">
        ...
        <button 
          onClick={onNext}
          className="bg-foreground text-background px-10 py-5 rounded-2xl font-black uppercase text-[10px] tracking-[0.4em] hover:bg-muted-foreground transition-all duration-300 shadow-2xl active:scale-95"
        >
