...
export const TestSeriesPreview = ({ isNested = false }: { isNested?: boolean }) => {
  return (
    <section className={isNested ? "h-full flex flex-col w-full relative pt-20 pb-16" : "py-32 bg-background border-b border-border"}>
      <div className="max-w-[1800px] w-full mx-auto px-4 md:px-12 flex flex-col h-full relative z-10">
        
        {/* Massive Title Centered Vertically */}
        <div className="flex-1 flex flex-col items-center justify-center">
          <span className="font-script text-4xl md:text-6xl text-foreground/40 lowercase mb-4 md:-mb-6 z-10 relative">the</span>
          <h2 className="text-[12vw] font-black uppercase tracking-tighter-editorial text-center leading-[0.85] text-foreground drop-shadow-xl select-none">
            Assessed <br /> Excellence
          </h2>
        </div>

        {/* Test Cards at the bottom */}
        <div className="grid md:grid-cols-3 gap-6 relative z-20 mt-12 w-full test-series-cards-container" style={{ perspective: '2000px' }}>
          {[
            ...
            <div key={index} className="test-series-card border border-foreground/10 bg-background/5 backdrop-blur-md p-8 group hover:bg-foreground/5 transition-all cursor-pointer flex flex-col items-start origin-bottom">
