...
import { FaSearch, FaFilter, FaFileExport, FaUserGraduate } from 'react-icons/fa';
...
export default function StreamRecordsPage() {
  ...
  // Capitalize standardized stream
      ...
      <FadeIn delay={0.1}>
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-card border border-border p-6 rounded-[2.5rem] shadow-sm">
            <div className="flex items-center gap-4 bg-muted/20 border border-border/50 px-6 py-4 rounded-2xl w-full md:w-96 focus-within:border-primary/40 focus-within:bg-background transition-all group">
               ...
               <input 
                 placeholder={`Search ${formatText(stream)} records...`} 
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
               <button className="flex-1 md:flex-none flex items-center justify-center gap-3 bg-muted/20 hover:bg-muted/40 text-foreground px-6 py-4 rounded-xl transition-all border border-border/40 font-bold text-xs">
                ...
                <button 
                  className="flex-none flex items-center justify-center bg-primary text-primary-foreground w-12 h-12 rounded-xl transition-all hover:opacity-90 shadow-lg shadow-primary/20 active:scale-95"
                  title={`Export ${formatText(stream)} Data`}
                >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-4">
               <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    <h3 className="text-xl font-black text-foreground tracking-tight">{formatText(standard)} Standard / {formatText(stream)} Ledger</h3>
                    <p className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest">Academic Session {year}</p>
                  </div>
               </div>
               <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-muted/20 border border-border/40">
                  <span className="text-[10px] font-black text-foreground uppercase tracking-widest">
                     {admissions.length} Students Enrolled
                  </span>
               </div>
            </div>
